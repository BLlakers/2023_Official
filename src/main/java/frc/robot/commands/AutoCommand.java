...
//} 
  ...
  if (Math.abs(m_DriveTrain.brDrive.getEncoder().getPosition()) < 25.2) { // Drives until the encoder is at rotations on the motor. 1 motor rotation = 8.14 wheel rotation 
      m_DriveTrain.drive(leftY, leftX, rightX);
    } else {
      m_DriveTrain.drive(() -> 0, () -> 0, () -> 0);
     // m_Shooter.shoot(1750, botRPM);
